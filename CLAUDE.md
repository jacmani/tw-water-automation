...
### Coverage Gap Check (`findCoverageGaps` in anthropic.ts) — added 2026-07-08

**Root cause of the 2026-07-08 incident** (technician had to manually re-enter tower DO totals; Section 2/6 shipped completely blank): `findDisagreements`/`findSourceDisagreements`/`findSummaryDisagreements` only fire on a **value mismatch** between the primary and an independent engine. If the primary returns `null` for a field and Qwen (or that day's HF fallback) **also** returns `null` — both engines genuinely failed to read that part of the sheet — the two nulls trivially "agree," no disagreement is ever recorded, the agreement gate passes, and the sheet ships with the gap silently unfilled. That's exactly what happened: Gemini returned `confidence=0.9` with all 7 `water_sources` totals null and `summary.input_total`/`tower_usage` null; the HF validator (Gemma-4-31B fallback that day) independently returned the same 0/7 sources and null summary for the same image. Zero disagreements → gate passed → no free tie-breaker, no paid escalation, dashboard shipped with Section 2 and Section 6 entirely blank.

Fix: `findCoverageGaps(result)` checks the **primary result alone** — no comparison needed — for `water_sources: 0/7 rows read` or a missing `summary.input_total`/`tower_usage`. A coverage gap now:
1. Fails the agreement gate on its own (folded into `gateClean`), even with zero cross-engine disagreements.
2. Triggers the free tie-breaker (OpenRouter → Groq) via `anyFreeDisagreement`.
3. `resolveWithTieBreaker` / `resolveSourceTieBreaker` / `resolveSummaryTieBreaker` were extended with a **coverage-fill** branch: if primary is null for a row/field, adopt whichever free engine (Qwen or the tie-breaker) actually has a reading, confidence capped at 0.7 and flagged — this used to only run when primary and another engine actively disagreed on a non-null value, never when primary was simply empty.
4. If the gap survives the free tie-breaker (every free engine missed the same section), `stillNeedsPaid` is forced `true` and Haiku escalation runs with a **targeted crop** of the actually-missing region (`middle` for source gaps, `bottom` for summary gaps) rather than defaulting to top+bottom.
5. An unconditional final recheck after the tie-breaker block also catches the case where `qwenResult` itself is unavailable (HF call failed outright), so a coverage gap can't slip through with the tie-breaker block skipped entirely.
...
**Google Cloud Vision is currently broken — needs the user, not Claude, to fix it (2026-07-08):** Vercel logs show every `[vision]` call failing with `403 PERMISSION_DENIED — This API method requires billing to be enabled` on GCP project #86038742207. This is a Google Cloud Console action (enabling billing = attaching a payment method to that project) that Claude must not perform — entering financial/billing credentials is out of scope regardless of the request. The pipeline degrades gracefully around this (Google Vision is word-level corroboration only, not primary extraction), so it did not cause the 2026-07-08 incident, but it's a real, currently-broken engine. **Action needed from the account owner:** visit `https://console.developers.google.com/billing/enable?project=86038742207` and enable billing (Google Cloud Vision has no billing-optional free tier at the project level, unlike Gemini's AI Studio key).

**HF Qwen3-VL-8B now dead on BOTH featherless-ai and novita (confirmed live 2026-07-08):** the `featherless-ai` candidate added earlier this session specifically to work around `novita` being dead is now *also* returning `400 model_not_supported — "not supported by any provider you have enabled"`. Checked `huggingface.co/settings/inference-providers/settings` directly — every provider (Novita, Featherless AI, Cerebras, etc.) is toggled **on** at the account level, so this isn't an account setting; the {model, provider} mapping itself has gone dark on HF's side for both providers. Not urgent: `HF_CANDIDATES` in `qwenVision.ts` already falls through to `Gemma-4-31B (novita)`, which is live and was used successfully. No code change made — documenting so a future session doesn't waste time in account settings before checking whether the fallback already covers it (it does).
