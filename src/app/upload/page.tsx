...
export default function UploadPage() {
  ...
  async function sharePoster() {
    ...
    setShareState('generating');
    try {
      // Two rAF cycles to ensure React has painted the off-screen template
  ...
  // Once poster data arrives and TemplateOverall has rendered, trigger the share
  useEffect(() => {
      ...
      <div className="flex-1 max-w-lg mx-auto w-full px-4 py-6">
        ...
        {status === 'success' && saveResult && (
          <div className="space-y-4">
            ...
            {/* Share / download overall poster */}
            {(shareState === 'generating' || shareState === 'ready' || shareState === 'error') && (
              <button
                onClick={sharePoster}
                disabled={shareState === 'generating' || !posterData}
                className="w-full flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-700 disabled:cursor-not-allowed text-white py-3 rounded-xl font-semibold transition-colors"
              >
