...
import type { CommitteeMember, TowerName, CommitteeRole, OFFICE_BEARER_ROLES } from '@/types';
...
function MemberCard({ member, accent }: { member: CommitteeMember; accent?: string }) {
  const borderColor = accent ?? '#475569';
  return (
    <div
    ...
    >
      ...
      {member.apartment && (
        <p className="text-slate-500 text-xs mt-1">Apt {member.apartment}</p>
      )}
      {member.phone && (
      ...
      <header className="bg-slate-900 border-b border-slate-800 px-4 py-4 sticky top-0 z-10">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div>
            ...
            {term && (
              <p className="text-slate-400 text-xs">Term {term}</p>
            )}
          </div>
          <Link
            href="/committee/admin"
            className="text-slate-400 hover:text-white text-xs border border-slate-700 hover:border-slate-500 px-3 py-1.5 rounded-lg transition-colors"
          >
            Admin
          </Link>
        </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {officeBearers.map((m) => (
                <MemberCard
                  key={m.id}
                  member={m}
                  accent={m.tower ? TOWER_COLORS[m.tower as TowerName] : '#6366f1'}
                />
              ))}
            </div>
          </section>
            ...
            <div className="space-y-5">
              {TOWERS.map((tower) => {
                ...
                return (
                  <div key={tower}>
                    <p
                      className={`text-xs font-semibold uppercase tracking-wider mb-2 ${TOWER_TEXT_CLASSES[tower]}`}
                    >
                      {tower}
                    </p>
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
        ...
        {members.length === 0 && (
          <div className="text-center py-16 text-slate-500">
            ...
            <p className="text-sm mt-1">
              <Link href="/committee/admin" className="text-blue-400 hover:text-blue-300">
                Add members in the admin page
              </Link>
            </p>
          </div>
        )}
